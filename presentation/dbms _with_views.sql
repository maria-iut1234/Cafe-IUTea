...
--
-- Stand-in structure for view `emp_man_view`
-- (See below for the actual view)
--
CREATE TABLE `emp_man_view` (
`e_id` varchar(100)
,`e_name` varchar(100)
...
--
-- Stand-in structure for view `inventories_view`
-- (See below for the actual view)
--
CREATE TABLE `inventories_view` (
`in_id` int(11)
,`in_name` varchar(100)
,`in_amount` int(100)
,`in_price` int(11)
...
(137, 45, 20, '2023-06-01', '2023-06-03');

-- --------------------------------------------------------

--
-- Stand-in structure for view `inv_man_view`
-- (See below for the actual view)
--
CREATE TABLE `inv_man_view` (
`in_id` int(11)
,`in_name` varchar(100)
,`in_amount` int(100)
,`in_price` int(11)
...
--
-- Stand-in structure for view `menu_man_view`
-- (See below for the actual view)
--
CREATE TABLE `menu_man_view` (
`menu_id` int(11)
,`menu_name` varchar(100)
,`menu_price` float
,`menu_pfp` varchar(100)
,`menu_cost` float
);
...
INSERT INTO `notifications` (`n_id`, `n_desc`, `n_reason`, `n_status`) VALUES
(37, 'Fresh Coffee Beans', 'Some Inventory Items have expired', 0),
(38, 'Beverages', 'Some Inventory Items will expire within 7 days', 0);

-- --------------------------------------------------------

--
-- Stand-in structure for view `notif_view`
-- (See below for the actual view)
--
CREATE TABLE `notif_view` (
`n_id` int(11)
,`n_desc` varchar(500)
,`n_reason` varchar(100)
,`n_status` tinyint(1)
);
...
--
-- Structure for view `emp_man_view`
--
DROP TABLE IF EXISTS `emp_man_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `emp_man_view`  AS SELECT `employee`.`e_id` AS `e_id`, `employee`.`e_name` AS `e_name`, `employee`.`e_password` AS `e_password`, `employee`.`e_pfp` AS `e_pfp`, `employee`.`e_dob` AS `e_dob`, `employee`.`e_address` AS `e_address`, `employee`.`e_email` AS `e_email` FROM `employee``employee`  ;

-- --------------------------------------------------------

--
-- Structure for view `inventories_view`
--
DROP TABLE IF EXISTS `inventories_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `inventories_view`  AS SELECT `inventories`.`in_id` AS `in_id`, `inventories`.`in_name` AS `in_name`, `inventories`.`in_amount` AS `in_amount`, `inventories`.`in_price` AS `in_price` FROM `inventories``inventories`  ;

-- --------------------------------------------------------

--
-- Structure for view `inv_man_view`
--
DROP TABLE IF EXISTS `inv_man_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `inv_man_view`  AS SELECT `inventories`.`in_id` AS `in_id`, `inventories`.`in_name` AS `in_name`, `inventories`.`in_amount` AS `in_amount`, `inventories`.`in_price` AS `in_price` FROM `inventories` ORDER BY `inventories`.`in_amount` ASC  ;

-- --------------------------------------------------------

--
-- Structure for view `menu_man_view`
--
DROP TABLE IF EXISTS `menu_man_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `menu_man_view`  AS SELECT `menu`.`menu_id` AS `menu_id`, `menu`.`menu_name` AS `menu_name`, `menu`.`menu_price` AS `menu_price`, `menu`.`menu_pfp` AS `menu_pfp`, `menu`.`menu_cost` AS `menu_cost` FROM `menu``menu`  ;

-- --------------------------------------------------------

--
-- Structure for view `notif_view`
--
DROP TABLE IF EXISTS `notif_view`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `notif_view`  AS SELECT `notifications`.`n_id` AS `n_id`, `notifications`.`n_desc` AS `n_desc`, `notifications`.`n_reason` AS `n_reason`, `notifications`.`n_status` AS `n_status` FROM `notifications``notifications`  ;
...
--
ALTER TABLE `inventory_orders`
  MODIFY `in_ord_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=139;
...
--
ALTER TABLE `notifications`
  MODIFY `n_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=39;
